...
def train(models, dataloader: DataLoader):
    ...
    while True:
        def train_step():
            ...
            loss = F.mse_loss(rgb1, pixels) #+ F.mse_loss(rgb0, pixels)
...
if __name__ == "__main__":
    ...
    dataloader = NeRFRayDataLoader(
        NeRFDataset(os.path.join("./dataset", "nerf_synthetic", "materials")),
        "train"
    )

    model_coarse.load_state_dict(torch.load(f"./models/lego_0_16000.pth"))
    model_coarse = model_coarse.to(device)
    model_fine.load_state_dict(torch.load(f"./models/lego_1_16000.pth"))
    ...
    for i in range(100):
        K, c2w, ray_origins, ray_dirs, img = dataloader.load_image(i, 0.6)

        ray_origins = ray_origins[None, ...]
        ray_dirs = ray_dirs[None, ...]

        with torch.no_grad():
            rgb0, rgb1 = render((model_coarse, model_fine), device, img.shape[:2], ray_origins, ray_dirs, num_samples=64, N_importance=128, perturb=False)
